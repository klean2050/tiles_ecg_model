...
class SongSplitter(Dataset):
    ...
    def __init__(self, dataset: Any, audio_length: int, overlap_ratio: float = 0) -> None:
        """Initialization.
    ...
    def __getitem__(self, idx: int) -> Tuple[Tensor, Tensor]:
        ...
        assert self.audio_length <= audio_song.size(dim=0), "audio_length is too large for song."

        # split song into segments:
        step = int(np.around((1 - self.overlap_ratio) * self.audio_length))
        audio_segments = audio_song.unfold(dimension=0, size=self.audio_length, step=step)
        # sanity check shape:
        assert audio_segments.dim() == 2 and audio_segments.size(dim=-1) == self.audio_length, "Error with shape of split song."
