...
class PTB_XL(data.Dataset):
    def __init__(self, root, sr, split="train"):
        ...
        if os.path.exists(f"data/ptb_xl/{split}_ecg.npy"):
            ...
            data_path = Path(self.root)  # .joinpath("physionet.org/files/ptb-xl/1.0.3")

            patient_df = pd.read_csv(
                data_path.joinpath("ptbxl_database.csv"), index_col=0
            )

            # Apple code
            patient_df = patient_df.dropna(subset=["site"])
            patient_df.scp_codes = patient_df.scp_codes.apply(
            ...
            mapping_df = pd.read_csv(
                data_path.joinpath("scp_statements.csv"), index_col=0
            )
            mapping_df = mapping_df[mapping_df.diagnostic == 1]
            patient_df["diagnostic_superclass"] = patient_df.scp_codes.apply(
                aggregate_diagnostic
            )

            # the suggested split from the data repo:
